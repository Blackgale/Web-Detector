...
function initCameraStream() {
  ...
  async function handleModel() {
    ...
    takePhotoButton.classList.toggle('loading')

    initializedModel = await roboflow
      .auth({
        publishable_key: 'rf_NYUnLixSxphTKfyZ8VkqlGVYoDs2',
      })
      .load({
        model: 'wall-detection-uv2p2',
        version: 1,
      })

    takePhotoButton.disabled = false
    takePhotoButton.classList.toggle('loading')
    console.log('The Roboflow model has been successfully initialized')
  }
...
function takeSnapshot() {
  ...
  initializedModel.detect(video)
    .then(function (predictions) {
      console.log('Predictions:')
      console.log(JSON.stringify(predictions, null, 4))

      predictions.forEach(prediction => {
        const { class: className, bbox, color, confidence } = prediction;
        ...
        let text = `${className} ${(confidence * 100).toFixed(2)}%`
        let font = '16px arial'

        context.font = font;
        context.textBaseline = 'top';
        let { width: textWidth } = context.measureText(text);
        console.log(textWidth)

        context.beginPath();
        context.fillStyle = color;
        context.fillRect(left, top, textWidth + 2, parseInt(font, 10) + 2);
        context.stroke();
    ...
    .then(function (canvas) {
      getCanvasBlob(canvas).then(function (blob) {
        saveAs(blob, `Wall Prediction ${(new Date()).toString()}.jpg`);
      });
    })
